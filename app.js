...
function renderEnvelope(envelope) {
  ...
  const difference = actual - envelope.planned;
  const status = Math.abs(difference) < .005 ? ['ok','Équilibré'] : difference > 0 ? ['over',`${formatter.format(difference)} au-dessus`] : ['under',`${formatter.format(Math.abs(difference))} disponible`];
  ...
  article.innerHTML = `
    <button class="envelope-main" type="button" aria-expanded="${envelope.open}">
      ...
      <span class="envelope-copy"><span class="envelope-name">${escapeHtml(envelope.name)}</span><span class="envelope-subtitle">${escapeHtml(envelope.subtitle || 'Sans description')}</span></span>
      <span class="envelope-amounts"><strong>${formatter.format(actual)}</strong><small>sur ${formatter.format(envelope.planned)}</small><span class="status ${status[0]}">${status[1]}</span></span>
  ...
  article.querySelector('.envelope-main').addEventListener('click', () => { envelope.open = !envelope.open; render(); });
